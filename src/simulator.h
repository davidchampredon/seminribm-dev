...
class simulator
{
	...
	// 1st column: Disease acquisition date for that infector
	// 2nd column: number of secondary cases generated by that infector
	// Number of row = population size
	dcMatrix _Reff;
	
	// dcMatrix of 'Who Infected Who' at different time points
...
public:
    ...
    vector<unsigned long>   get_nS();
    vector<unsigned long>   get_nR();
    vector<vector<double> > get_Reff();
    
    vector<vector<double> > get_GIfwd();
    vector<double>          get_GIbck();
    ...
    vector<double>  get_timeDiseaseAcquisition();
    vector<double>  get_timeDiseaseAcquisition_transm();
